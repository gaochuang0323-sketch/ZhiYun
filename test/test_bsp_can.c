...
void test_Process_receives_extended_id(void)
{
    ...
    FDCAN_SetRxFifoFillLevel(1);
    FDCAN_SetRxData(0x1FFFFFFF, 1, FDCAN_DLC_BYTES_8, rxData);

    BspCan_Process();

    BspCan_Status status = BspCan_GetStatus();
    TEST_ASSERT_EQUAL_UINT32(1, status.rxCount);
    TEST_ASSERT_EQUAL_UINT32(0x1FFFFFFF, status.lastRxId);
    TEST_ASSERT_EQUAL_UINT8(1, status.lastRxIsExtended);
}

void test_Process_tick_is_recorded(void)
{
    uint8_t rxData[1] = {0xAA};

    HAL_SetTick(12345);
    FDCAN_SetRxFifoFillLevel(1);
    FDCAN_SetRxData(0x100, 0, FDCAN_DLC_BYTES_1, rxData);

    BspCan_Process();

    TEST_ASSERT_EQUAL_UINT32(12345, BspCan_GetStatus().lastRxTick);
}

void test_Process_multiple_frames_all_drained(void)
{
    uint8_t rxData1[8] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
    uint8_t rxData2[8] = {0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80};

    FDCAN_SetRxFifoFillLevel(3);
    FDCAN_SetRxData(0x100, 0, FDCAN_DLC_BYTES_8, rxData1);

    BspCan_Process();

    BspCan_Status status = BspCan_GetStatus();
    TEST_ASSERT_EQUAL_UINT32(3, status.rxCount);
}

void test_SendClassic_not_started_returns_error(void)
{
    BspCan_Status status;
    FDCAN_SetStartResult(HAL_ERROR);
    BspCan_Init();

    status = BspCan_GetStatus();
    TEST_ASSERT_EQUAL_UINT8(0, status.started);

    uint8_t data[1] = {0};
    HAL_StatusTypeDef result = BspCan_SendClassic(0x100, 0, data, 1);
